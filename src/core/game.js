...
export class Game { constructor(canvas, input, ui, audio) { this.state = new State(); this.input = input; this.ui = ui; this.audio = audio; this.canvas = canvas; this.effects = new Effects(); this.menu = ui.menu; this.pause = ui.pause; this.over = ui.over; this.bindUI(); this.reset(); }
  reset() { this.world = new World(); this.player = new Player(SPAWN); this.weapon = new Weapon(WEAPONS.sidearm, this.audio); this.wave = 1; this.waveClearTimer = 0; this.camera = new Camera(this.player); this.player.setCamera(this.camera); this.renderer = new Renderer(this.canvas, this.camera, new Raycaster(this.world), this.ui.hud); this.spawnWave(); }
